...
font.load(function(err){
	var path = font.toPath("Aa", r.width/2 - 100, r.height/2, 200)
	.fill(0)
	.stroke(false)
	...
	diseaseAesthetics: function()
	{
		...
		{	
			...
			}else
			{
				...
				for(var i = 0; i < 2; i++){
					if(i % 2 == 0){
						r.rect(this.x - this.infectionSize/2, this.y - this.infectionSize/2, 20 + i * 800, 20 + i * 800)
						 .fill(false)
						 .stroke(Rune.random(255))
						 .strokeWidth(Rune.random(0.5))//(0.1)
...
//	--------------------------------------------------------------------------------------------------------------
	...
	for(var i = 1; i < cellArray.length - 1; i++) {
		...
		var neighbourRight = cellArray[i+1];
		 ...
		 if(neighbourLeft.infected) cellArray[i].infectionSize++; //+= Rune.random(0.5, 1.5);
		 if(neighbourRight.infected) cellArray[i].infectionSize++; //+= Rune.random(0.5, 1.5);

		cellArray[i].diseaseAesthetics();
	}

	//infectionThreshold += 2;
}